...
Browser
  → Railway / Next.js server
      ...
      → AI provider: live web research and product-level forecast output
...
3. Ask the configured AI provider to research exactly the active variables declared by the skill; the starter policy includes examples that admins may add, rename, or remove.
4. Require sources and structured evidence. Web content is untrusted research material, never app instructions.
5. Produce a product-level forecast with quantities, rationale, confidence, and evidence.
6. Store the result, inputs, skill version, provider/model, evidence links, and run status for review.
7. Build one combined email containing only the recipient's assigned locations.
...
- `ForecastProvider` for web-capable AI analysis and cited structured output
